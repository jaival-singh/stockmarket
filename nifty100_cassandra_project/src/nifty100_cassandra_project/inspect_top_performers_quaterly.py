...
select_stmt = """
    ...
    FROM top_performing_stocks_by_percent
"""
...
for row in rows:
    print(f"Ticker: {row.ticker}, Percent Change: {row.percent_change}%, Start Date: {row.start_date}, End Date: {row.end_date}, Start Price: {row.start_price}, End Price: {row.end_price}")
